...
def dict_to_psetdef(check,OTL_psetinfoDict): 
    if check == "geldig":
        with adoc.LockDocument():
            with adoc.Database as db:
                with db.TransactionManager.StartTransaction() as t:
        ...
        info = str(pdn_count) + " Properties aangemaakt in " + str(psetdef_count) + " Propertysets"

        if len(outputlijst) < 30:
            ctypes.windll.user32.MessageBoxW(0, str(info) + "\n Volgende sets werden aangemaakt \n" + str(outputlijst), "Propertysets aangemaakt", 0)
        else:
            outputlijst = outputlijst[:30]
            ctypes.windll.user32.MessageBoxW(0, str(info) + "\n Onder andere volgende sets werden aangemaakt \n" + str(outputlijst) + "...", "Propertysets aangemaakt", 0)
        return [info,outputlijst]

    else:
        ctypes.windll.user32.MessageBoxW(0, "\n Geen propertysets aangemaakt \n Ongeldige OTL db", "Propertysets aangemaakt", 0)
        return ["ongeldige OTL db"]
